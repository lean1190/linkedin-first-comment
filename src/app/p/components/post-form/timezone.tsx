...
export default function Timezone({ schedule, viewport }: Props) {
    const timeInPacific = useMemo(() => transformDateToTimezone({
        dateString: schedule,
        targetTimezone: 'America/Los_Angeles'
    }), [schedule]);

    const timeInArgentina = useMemo(() => transformDateToTimezone({
        dateString: schedule,
        targetTimezone: 'America/Argentina/Buenos_Aires'
    }), [schedule]);

    const timeInBerlin = useMemo(() => transformDateToTimezone({
        dateString: schedule,
        targetTimezone: 'Europe/Berlin'
    }), [schedule]);

    const timeInKualaLumpur = useMemo(() => transformDateToTimezone({
        dateString: schedule,
    ...
    return (
        <details className="text-sm text-linkedin-low-emphasis [&_i]:open:-rotate-180">
            ...
            <div className="py-4 font-thin">
                <TimezoneLine schedule={timeInPacific} timezone="America/Los_Angeles" showLocation={false} />
                <TimezoneLine schedule={timeInArgentina} timezone="America/Argentina/Buenos_Aires" showLocation={false} />
                <TimezoneLine schedule={timeInBerlin} timezone="Europe/Berlin" showLocation={false} />
                <TimezoneLine schedule={timeInKualaLumpur} timezone="Asia/Kuala_Lumpur" showLocation={false} />
            </div>
        </details>
    );
}
